...
def render(prs, routed_content):
    ...
    for slide_obj in routed_content.slides:
        ...
        title_box = slide.shapes.add_textbox(Inches(0.6), Inches(0.22), Inches(9.0), Inches(1.0))
        ...
        # ===== 右上角品牌标签：固定 =====
        brand_tag = "CARI北美前沿"
        tag = slide.shapes.add_shape(MsoShape.RECTANGLE, Inches(8.0), Inches(0.22), Inches(2.2), Inches(0.42))
        tag.fill.solid(); tag.fill.fore_color.rgb = RGBColor(255,255,255)
        ...
        sum_bullets = norm_bullets(val(summary, "bullets"), mi=2, ma=4)

        # 高度更紧凑
        block_h = Inches(0.9 + (len(sum_bullets)-1)*0.25)
        block_top = Inches(1.05)
        full_left = Inches(0.6)
        full_w = Inches(10) - Inches(1.2)

        # 灰框：无边框
        bg = slide.shapes.add_shape(MsoShape.RECTANGLE, full_left, block_top, full_w, block_h)
        bg.fill.solid(); bg.fill.fore_color.rgb = RGBColor(230,230,230)
        ...
        stops[0].color.rgb = RGBColor(180,40,40)   # 左上浅红
        stops[1].color.rgb = RGBColor(120,0,0)    # 右下深红
        red.line.fill.background()
        no_shadow(red)

        # 红条文字：启示 / 洞察，垂直水平居中
        l1, l2 = split_two_lines(sum_label)
        tf_red = red.text_frame; tf_red.clear()
        tf_red.vertical_anchor = MSO_ANCHOR.MIDDLE
        fs = 16
        p1 = tf_red.paragraphs[0]; p1.text = l1; p1.font.bold = True; p1.font.size = Pt(fs)
        p1.font.color.rgb = RGBColor(255,255,255); p1.alignment = PP_ALIGN.CENTER
        p2 = tf_red.add_paragraph(); p2.text = l2; p2.font.bold = True; p2.font.size = Pt(fs)
        p2.font.color.rgb = RGBColor(255,255,255); p2.alignment = PP_ALIGN.CENTER

        # 右侧 bullets（冒号前加粗，内容更长，上下居中）
        ...
        tf.vertical_anchor = MSO_ANCHOR.MIDDLE  # 整体上下居中

        for i, b in enumerate(sum_bullets):
            # 如果 bullet 里有“：”，则冒号前加粗
            if "：" in b:
                title_part, rest = b.split("：", 1)
                text = f"- {title_part}：{rest.strip()}"
                pp = tf.add_paragraph() if i > 0 else tf.paragraphs[0]
                run_bold = pp.add_run()
                run_bold.text = f"- {title_part}："  # 总结部分
                run_bold.font.bold = True
                run_bold.font.size = Pt(12)
                run_bold.font.color.rgb = theme.colors["text"]

                run_rest = pp.add_run()
                run_rest.text = rest.strip()
                ...
                pp.text = f"- {b}"
                pp.font.size = Pt(12)
                pp.font.color.rgb = theme.colors["text"]
        ...
        def draw_column(col_left, col_data, fallback="列"):
            ...
            # 主标题
            tag_w = Inches(3.5); tag_h = Inches(0.36)
            tag_x = col_left + (col_w - tag_w) / 2
            tag_y = grid_top - tag_h/2
            tag = slide.shapes.add_shape(MsoShape.RECTANGLE, tag_x, tag_y, tag_w, tag_h)
            tag.fill.solid(); tag.fill.fore_color.rgb = RGBColor(255,255,255)
            tag.line.fill.background(); no_shadow(tag)
            tf_tag = tag.text_frame; pt = tf_tag.paragraphs[0]
            pt.text = title_text; pt.font.size = Pt(12); pt.font.bold = True
            pt.font.color.rgb = RGBColor(0,0,0); pt.alignment = PP_ALIGN.CENTER

            # 内文 sections
            tfb = box.text_frame; tfb.clear(); tfb.word_wrap = True
            for idx, sec in enumerate(sections):
                subtitle = val(sec, "subtitle_bold", "") or f"小节{idx+1}"
                bullets = norm_bullets(val(sec, "bullets"), mi=4, ma=6)

                # subtitle_bold：黑色，加粗，12pt
                psec = tfb.add_paragraph()
                psec.text = f"□ 【{subtitle}】"
                psec.font.size = Pt(12); psec.font.bold = True
